        ...
        if (response.ok){
            ...
            let randomHex = Math.floor(Math.random() * 0xffffff).toString(16);
             randomHex = `#${randomHex.padStart(6, "0")}`;
                ...
                document.querySelector(".data_list_hold").innerHTML+=`<li class="items" key="${data_list[i].name}${data_list[i].phoneNumber}">
                 <div class="photo">
...
async function fetch_contacts(){
    try {
       ...
       console.table(data_list)
       if (response.ok){
           document.querySelector(".contacts2").innerHTML=`<div class="each_contacts"></div>`
           for(let i=0; i<data_list.length; i++){
           let randomHex = Math.floor(Math.random() * 0xffffff).toString(16);
            randomHex = `#${randomHex.padStart(6, "0")}`;
            
            
               document.querySelector(".each_contacts").innerHTML+=`<div class="contact_item" key="${data_list[i].name}${data_list[i].phoneNumber}">
                <div class="photo">
...
async function addCustomers(){
    ...
    try {
        ...
        if (checkPhoneValid() && !nums.includes(cpd)){
            ...
            if (response.ok) {
                fetch_data()
                alertmsg("success","lightgreen")
            }else{
                alertmsg("failed","rgb(183, 1, 1)")
            }
