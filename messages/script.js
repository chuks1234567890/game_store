...
async function fetch_data(){
    ...
    try {
         const request=new Request("https://smsprovider-1.onrender.com/api/v1/customers/all",{
             // mode:"no-cors",
             headers:{
         ...
         const my_customers=await info.msg
         // console.table(my_customers)
         // ul_holder.innerHTML+=my_customers.map(item=>(
         //     ``
         // ))
         document.querySelectorAll(".loader").forEach(item=>{
         ...
         for (let i=0; i<my_customers.length ; i++){
             ul_holder.innerHTML+=`<li class="items" key="${i + 7}">
                 <div class="photo">
 
                 </div>
                 <div class="details">
                     <span class="user">${my_customers[i].name}</span>
                     <span class="user">${my_customers[i].phoneNumber}</span>
                     <div class="ooptn">
                         <div class="a" onclick='delete_customer("${my_customers[i].phoneNumber}","${my_customers[i].name}",${i + 7})'>
             ...
             let randomHex = Math.floor(Math.random() * 0xffffff).toString(16);
             randomHex = `#${randomHex.padStart(6, "0")}`;
             all_data=document.querySelectorAll(".photo")
             nums.push(my_customers[i].phoneNumber)
             all_data[i].style.background=randomHex
             all_data[i].innerHTML=`<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M5 21C5 17.134 8.13401 14 12 14C15.866 14 19 17.134 19 21M16 7C16 9.20914 14.2091 11 12 11C9.79086 11 8 9.20914 8 7C8 4.79086 9.79086 3 12 3C14.2091 3 16 4.79086 16 7Z" stroke="rgba(0,0,0,.4)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>`
             }
    } catch (errr) {
